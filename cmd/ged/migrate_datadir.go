...
// migrateExistingDirToClassicNamingScheme renames default base data directory ".../Earthdollar" to ".../Earthdollar", pending os customs, etc... ;-)
///
// Check for preexisting **Un-classic** data directory, ie "/home/path/to/Earthdollar".
// If it exists, check if the data therein belongs to Classic blockchain (ie not configged as "ETF"),
// and rename it to fit Classic naming convention ("/home/path/to/Earthdollar") if that dir doesn't already exist.
...
// migrateToChainSubdirIfNecessary migrates ".../Earthdollar/nodes|chaindata|...|nodekey" --> ".../Earthdollar/mainnet/nodes|chaindata|...|nodekey"
func migrateToChainSubdirIfNecessary(ctx *cli.Context) error {
	chainIdentity := mustMakeChainIdentity(ctx) // "mainnet", "morden", "custom"

	datapath := mustMakeDataDir(ctx) // ".../Earthdollar/ | --datadir"

	subdirPath := MustMakeChainDataDir(ctx) // ie, <Earthdollar>/mainnet
	...
	if subdirPathInfo != nil && !subdirPathInfo.IsDir() {
		return fmt.Errorf(`%v: found file named '%v' in Earthdollar datadir,
	...
	if chainIdentitiesMorden[chainIdentity] {
		...
		if !exTestDirInfo.IsDir() {
			return nil // don't interfere with user *file* that won't be relevant for ged
		}
	...
	for _, dir := range []string{"dapp", "keystore", "chaindata", "nodes"} {
		...
		if !dirInfo.IsDir() {
			continue // don't interfere with user *file* that won't be relevant for ged
		}
	...
	for _, file := range []string{"nodekey", "ged.ipc"} {
		...
		if fileInfo.IsDir() {
			continue // don't interfere with user dirs that won't be relevant for ged
		}
