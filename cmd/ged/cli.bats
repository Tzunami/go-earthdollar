...
@test "runs with valid flag and valid command" {
	run $GED_CMD --datadir $DATA_DIR --maxpeers 0 --nodiscover --nat none --ipcdisable --exec 'exit' console
	echo "$output"
...
@test "displays help with invalid flag and valid command" {
	# --nodisco
	run $GED_CMD --datadir $DATA_DIR --maxpeers 0 --nodisco --nat none --ipcdisable --exec 'exit' console
	echo "$output"
...
@test "displays help with valid flag and invalid command" {
	# conso
	run $GED_CMD --datadir $DATA_DIR --maxpeers 0 --nodiscover --nat none --ipcdisable --exec 'exit' conso
	echo "$output"
...
@test "exactly overlapping flags not allowed: --chain=morden --testnet" {
	run $GED_CMD --data-dir $DATA_DIR --testnet --chain=morden --maxpeers 0 --nodiscover --nat none --ipcdisable --exec 'exit' console
	echo "$output"
	[ "$status" -ne 0 ]
}

@test "custom testnet subdir --testnet --chain=morden2 | exit !=0" {
	run $GED_CMD --data-dir $DATA_DIR --testnet --chain=morden2 --maxpeers 0 --nodiscover --nat none --ipcdisable --exec 'exit' console
	echo "$output"
	[ "$status" -ne 0 ]	
	[[ "$output" == *"invalid flag "* ]]
}
...
@test "aliasing directory flags: --data-dir==--datadir, --ipc-path==--ipcpath" {
	...
	run $GED_CMD --datadir $DATA_DIR --keystore $DATA_DIR/keyhere console
	[ "$status" -eq 0 ]
	[ -d $DATA_DIR/keyhere ]

	# data-dir/datadir
	run $GED_CMD --datadir $DATA_DIR console
	[ "$status" -eq 0 ]
	[ -d $DATA_DIR ]

	run $GED_CMD --data-dir $DATA_DIR console
	[ "$status" -eq 0 ]
	[ -d $DATA_DIR ]

	# # ipc-path/ipcpath
	run $GED_CMD --data-dir $DATA_DIR --ipc-path abc.ipc console
	[ "$status" -eq 0 ]
	[[ "$output" == *"IPC endpoint opened: $DATA_DIR/mainnet/abc.ipc"* ]]

	run $GED_CMD --data-dir $DATA_DIR --ipcpath $DATA_DIR/mainnet/abc.ipc console
	[ "$status" -eq 0 ]
	[[ "$output" == *"IPC endpoint opened: $DATA_DIR/mainnet/abc.ipc"* ]]

	run $GED_CMD --data-dir $DATA_DIR --ipc-path $DATA_DIR/mainnet/abc console
	[ "$status" -eq 0 ]
	[[ "$output" == *"IPC endpoint opened: $DATA_DIR/mainnet/abc"* ]]

	run $GED_CMD --data-dir $DATA_DIR --ipcpath $DATA_DIR/mainnet/abc console
	[ "$status" -eq 0 ]
	[[ "$output" == *"IPC endpoint opened: $DATA_DIR/mainnet/abc"* ]]
...
@test "aliasing hyphenated flags: --no-discover==--nodiscover, --ipc-disable==--ipcdisable | exit 0" {
	...
	do
		...
		run $GED_CMD --datadir $DATA_DIR --$var --exec 'exit' console
		[ "$status" -eq 0 ]
		[[ "$output" == *"Starting"* ]]
		[[ "$output" == *"Blockchain DB Version: "* ]]
		[[ "$output" == *"Starting Server"* ]]
	...
	do
		run $GED_CMD --data-dir $DATA_DIR --$var --exec 'exit' console
		[ "$status" -eq 0 ]
		[[ "$output" == *"Starting"* ]]
		[[ "$output" == *"Blockchain DB Version: "* ]]
		[[ "$output" == *"Starting Server"* ]]
...
@test "--cache 16 | exit 0" {
	run $GED_CMD --data-dir $DATA_DIR --cache 17 console
	[ "$status" -eq 0 ]
...
@test "dump [noargs] | exit >0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/ 
	run $GED_CMD --data-dir $DATA_DIR dump
	echo "$output"
	[ "$status" -gt 0 ]
...
@test "dump 0 [noaddress] | exit 0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/
	run $GED_CMD --data-dir $DATA_DIR dump 0
	echo "$output"
	[ "$status" -eq 0 ]
...
@test "dump 0 fff7ac99c8e4feb60c9750054bdc14ce1857f181 | exit 0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/
	run $GED_CMD --data-dir $DATA_DIR dump 0 fff7ac99c8e4feb60c9750054bdc14ce1857f181
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"root"* ]] # block state root
	[[ "$output" == *"balance"* ]]
...
@test "dump 0 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f | exit 0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/
	run $GED_CMD --data-dir $DATA_DIR dump 0 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"root"* ]] # block state root
	[[ "$output" == *"balance"* ]]
...
@test "dump 0,1 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f | exit 0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/
	run $GED_CMD --data-dir $DATA_DIR dump 0,1 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"root"* ]] # block state root
	[[ "$output" == *"balance"* ]]
...
@test "dump 0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f | exit 0" {
	cp -a $BATS_TEST_DIRNAME/../../cmd/ged/testdata/testdatadir/. $DATA_DIR/
	run $GED_CMD --data-dir $DATA_DIR dump 0xd4e56740f876aef8c010b86a40d5f56745a118d0906a34e69aec8c0db1cb8fa3 fff7ac99c8e4feb60c9750054bdc14ce1857f181,0xffe8cbc1681e5e9db74a0f93f8ed25897519120f
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"root"* ]] # block state root
	[[ "$output" == *"balance"* ]]
...
@test "status command present and true for mainnet" {
	run $GED_CMD --data-dir $DATA_DIR status
	[ "$status" -eq 0 ]
	...
	run $GED_CMD --data-dir $DATA_DIR --exec 'exit' console # set up db
	[ "$status" -eq 0 ]
	run $GED_CMD --data-dir $DATA_DIR status
	[ "$status" -eq 0 ]
...
@test "status command present and true for morden" {
	run $GED_CMD --data-dir $DATA_DIR --chain morden status
	[ "$status" -eq 0 ]
	...
	# Run twice, because the second time will have set up database.
	run $GED_CMD --data-dir $DATA_DIR --chain morden --exec 'exit' console # set up db
	[ "$status" -eq 0 ]
	run $GED_CMD --data-dir $DATA_DIR --chain morden status
	[ "$status" -eq 0 ]
