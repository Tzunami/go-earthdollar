...
@test "dump-chain-config | exit 0" {
	run $GED_CMD --datadir $DATA_DIR --maxpeers 0 dump-chain-config $DATA_DIR/dump.json
	echo "$output"
...
@test "--testnet dump-chain-config | exit 0" {
	run $GED_CMD --datadir $DATA_DIR --testnet dump-chain-config $DATA_DIR/dump.json
	echo "$output"
...
@test "--chain morden dump-chain-config | exit 0" {
	run $GED_CMD --datadir $DATA_DIR --chain morden dump-chain-config $DATA_DIR/dump.json
	echo "$output"
...
@test "--chain kittyCoin dump-chain-config | exit !=0" {
	run $GED_CMD --datadir $DATA_DIR --chain kittyCoin dump-chain-config $DATA_DIR/dump.json
	echo "$output"
	[ "$status" -ne 0 ]
}
...
@test "dump-chain-config | --chain | exit 0" {
	...
	run $GED_CMD --datadir $DATA_DIR dump-chain-config "$customnet"/chain.json
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"Wrote chain config file"* ]]
...
@test "dump-chain-config | --chain privatenet.json | exit 0" {
	...
	run $GED_CMD --data-dir $DATA_DIR --chain morden dump-chain-config $DATA_DIR/privatenet.json
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"Wrote chain config file"* ]]
...
@test "--chain morden dump-chain-config | --chain == morden | exit 0" {
	...
	run $GED_CMD --datadir $DATA_DIR --chain=morden dump-chain-config "$customnet"/chain.json
	echo "$output"
	[ "$status" -eq 0 ]
	[[ "$output" == *"Wrote chain config file"* ]]
...
@test "--chain morden dump-chain-config | --chain -> kitty | exit 0" {
	...
	run $GED_CMD --datadir $DATA_DIR --chain kitty status
	[ "$status" -eq 0 ]
	echo "$output"
...
@test "--chain kitty --testnet | exit !=0" {
	...
	run $GED_CMD --data-dir $DATA_DIR --chain kitty  --testnet console
	echo "$output"
	[ "$status" -ne 0 ]
...
@test "--chain kitty @ sed -i s/key/badkey/ kitty/chain.json | exit !=0" {
	...
	do
		...
		run $GED_CMD --datadir $DATA_DIR --chain kitty console
		echo "$output"
		[ "$status" -ne 0 ]
		if [ ! "$status" -ne 0 ]; then
