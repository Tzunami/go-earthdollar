...
@test "defaults: genesis block hash mainnet constant @ _" {
	run $GED_CMD --data-dir $DATA_DIR --exec 'ed.getBlock(0).hash' console
	echo "$output"
...
@test "defaults: genesis block hash constant @ --chain=morden" {
	run $GED_CMD --chain=morden --data-dir $DATA_DIR --exec 'ed.getBlock(0).hash' console
	echo "$output"
...
@test "defaults: genesis block hash constant @ --chain=testnet" {
	run $GED_CMD --chain=testnet --data-dir $DATA_DIR --exec 'ed.getBlock(0).hash' console
	echo "$output"
...
@test "defaults: genesis block hash constant @ --testnet" {
	run $GED_CMD --testnet --data-dir $DATA_DIR --exec 'ed.getBlock(0).hash' console
	echo "$output"
