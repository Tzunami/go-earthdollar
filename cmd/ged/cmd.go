...
func formatEdConfigPretty(edConfig *ed.Config) (s []string) {
	ss := []printable{}

	// NetworkID
	ss = append(ss, printable{0, "Network", edConfig.NetworkId})
	// FastSync?
	ss = append(ss, printable{0, "Fast sync", edConfig.FastSync})
	// BlockChainVersion
	ss = append(ss, printable{0, "Blockchain version", edConfig.BlockChainVersion})
	// DatabaseCache
	ss = append(ss, printable{0, "Database cache (MB)", edConfig.DatabaseCache})
	// DatabaseHandles
	ss = append(ss, printable{0, "Database file handles", edConfig.DatabaseHandles})
	// NatSpec?
	ss = append(ss, printable{0, "NAT spec", edConfig.NatSpec})
	// AutoDAG?
	ss = append(ss, printable{0, "Auto DAG", edConfig.AutoDAG})
	// PowTest?
	ss = append(ss, printable{0, "Pow test", edConfig.PowTest})
	// PowShared?
	ss = append(ss, printable{0, "Pow shared", edConfig.PowShared})
	// SolcPath
	ss = append(ss, printable{0, "Solc path", edConfig.SolcPath})

	// Account Manager
	lenAccts := len(edConfig.AccountManager.Accounts())
	...
	ss = append(ss, printable{0, "GPO max gas price", edConfig.GpoMaxGasPrice})
	// MinerThreads
	ss = append(ss, printable{0, "Miner threads", edConfig.MinerThreads})
...
func status(ctx *cli.Context) error {
	...
	edConf := mustMakeEdConf(ctx, config)
	...
	printme := []struct {
		...
		{"Earthdollar configuration", formatEdConfigPretty(edConf)},
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node) *ed.Earthdollar {
...
func runStatusSyncLogs(e *ed.Earthdollar, interval string, maxPeers int) {
