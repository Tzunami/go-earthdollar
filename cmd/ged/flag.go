...
// --> <home>/<Earthdollar>(defaulty) or --datadir
...
// --> <home>/<Earthdollar>/<mainnet|testnet|custom-net>, per --chain
...
func MakeSystemNode(version string, ctx *cli.Context) *node.Node {
	...
	if shouldAttemptDirMigration(ctx) {
		// Rename existing default datadir <home>/<Earthdollar>/ to <home>/<Earthdollar>.
		// Only do this if --datadir flag is not specified AND <home>/<Earthdollar> does NOT already exist (only migrate once and only for defaulty).
		// If it finds an 'Earthdollar' directory, it will check if it contains default ETC or ETHF chain data.
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		glog.Fatalf("%v: failed to register the Earthdollar service: ", ErrStackFail, err)
	}
	if shhEnable {
...
func mustMakeEdConf(ctx *cli.Context, sconf *core.SufficientChainConfig) *ed.Config {
	...
	if _, ok := edConf.GasPrice.SetString(ctx.GlobalString(aliasableName(GasPriceFlag.Name, ctx)), 0); !ok {
	...
	if _, ok := edConf.GpoMinGasPrice.SetString(ctx.GlobalString(aliasableName(GpoMinGasPriceFlag.Name, ctx)), 0); !ok {
	...
	if _, ok := edConf.GpoMaxGasPrice.SetString(ctx.GlobalString(aliasableName(GpoMaxGasPriceFlag.Name, ctx)), 0); !ok {
