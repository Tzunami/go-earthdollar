...
@test "account list testdata keystore (db)" {
	...
	# Create index.
	run $GED_CMD --data-dir $DATA_DIR --index-accounts account index
	[ "$status" -eq 0 ]

	run $GED_CMD --datadir $DATA_DIR --index-accounts account
	echo "$output"
...
@test "account create (db)" {
	run $GED_CMD --datadir $DATA_DIR --lightkdf --index-accounts account new <<< $'secret\nsecret\n'
	echo "$output"
...
@test "account update pass (db)" {
	cp -R $BATS_TEST_DIRNAME/../../accounts/testdata/keystore $DATA_DIR/mainnet
	run $GED_CMD --keystore $DATA_DIR/mainnet/keystore --index-accounts account index
	[ "$status" -eq 0 ]
	...
	run $GED_CMD --datadir $DATA_DIR --lightkdf --index-accounts account update f466859ead1932d743d622cb74fc058882e8648a <<< $'foobar\nother\nother\n'
	echo "$output"
...
@test "account import (db)" {
	run $GED_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $BATS_TEST_DIRNAME/testdata/guswallet.json <<< $'foo\n'
	echo "$output"
...
@test "account import pass mismatch (db)" {
	run $GED_CMD --datadir $DATA_DIR --lightkdf --index-accounts wallet import $BATS_TEST_DIRNAME/testdata/guswallet.json <<< $'wrong\n'
	echo "$output"
...
@test "account unlock (db)" {
	...
	run $GED_CMD --datadir $DATA_DIR --index-accounts --keystore="$DATA_DIR"/mainnet/keystore\
...
@test "account unlock multiple with wrong pass file (db)" {
	...
	run $GED_CMD --datadir $DATA_DIR --nat none --index-accounts --keystore="$DATA_DIR"/mainnet/keystore\
	 --nat none --nodiscover --dev --password $DATA_DIR/pass.txt --unlock 0,2 js $DATA_DIR/empty.js
	echo "$output"
...
@test "account unlock ambiguous (db)" {
	...
	run $GED_CMD --keystore $DATA_DIR/mainnet/store --index-accounts account index
	[ "$status" -eq 0 ]

	run $GED_CMD --datadir $DATA_DIR --keystore $DATA_DIR/mainnet/store --index-accounts --keystore="$DATA_DIR"/mainnet/store\
	 --nat none --nodiscover --dev --unlock f466859ead1932d743d622cb74fc058882e8648a js $DATA_DIR/empty.js <<< $'foobar\n'$DATA_DIR/store/1
	echo "$output"
