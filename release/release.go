...
type Config struct {
	Oracle common.Address // Earthdollar address of the release oracle
	...
	for {
		...
		case <-timer.C:
			...
			if version.Major > r.config.Major ||
				...
				howtofix := fmt.Sprintf("Please check https://github.com/Tzunami/go-earthdollar/releases for new releases")
