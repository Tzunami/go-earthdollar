#include <iomanip>
#include <libedhash/fnv.h>
#include <libedhash/edhash.h>
#include <libedhash/internal.h>
#include <libedhash/io.h>

#ifdef WITH_CRYPTOPP

#include <libedhash/sha3_cryptopp.h>

#else
#include <libedhash/sha3.h>
#endif // WITH_CRYPTOPP
...
// some functions taken from ed::dev for convenience.
...
std::string blockhashToHexString(edhash_h256_t* _hash)
{
...
edhash_h256_t stringToBlockhash(std::string const& _s)
{
	edhash_h256_t ret;
...
BOOST_AUTO_TEST_CASE(test_edhash_io_mutable_name) {
	char mutable_name[DAG_MUTABLE_NAME_MAX_SIZE];
	// should have at least 8 bytes provided since this is what we test :)
	edhash_h256_t seed1 = edhash_h256_static_init(0, 10, 65, 255, 34, 55, 22, 8);
	edhash_io_mutable_name(1, &seed1, mutable_name);
	BOOST_REQUIRE_EQUAL(0, strcmp(mutable_name, "full-R1-000a41ff22371608"));
	edhash_h256_t seed2 = edhash_h256_static_init(0, 0, 0, 0, 0, 0, 0, 0);
	edhash_io_mutable_name(44, &seed2, mutable_name);
	BOOST_REQUIRE_EQUAL(0, strcmp(mutable_name, "full-R44-0000000000000000"));
}

BOOST_AUTO_TEST_CASE(test_edhash_dir_creation) {
	edhash_h256_t seedhash;
	FILE *f = NULL;
	...
	// cleanup
	fclose(f);
	fs::remove_all("./test_edhash_directory/");
}

BOOST_AUTO_TEST_CASE(test_edhash_io_memo_file_match) {
	uint64_t full_size;
	uint64_t cache_size;
	edhash_h256_t seed;
	edhash_h256_t hash;
	...
	edhash_light_t light = edhash_light_new_internal(cache_size, &seed);
	edhash_full_t full = edhash_full_new_internal(
		"./test_edhash_directory/",
		seed,
		full_size,
		light,
		NULL
...
BOOST_AUTO_TEST_CASE(test_edhash_io_memo_file_size_mismatch) {
...
BOOST_AUTO_TEST_CASE(test_edhash_get_default_dirname) {
	char result[256];
	...
	BOOST_REQUIRE(edhash_get_default_dirname(result, 256));
	std::string res = std::string(homedir) + std::string("/.edhash/");
#endif
...
BOOST_AUTO_TEST_CASE(light_and_full_client_checks) {
	...
	edhash_h256_t seed;
	edhash_h256_t hash;
	edhash_h256_t difficulty;
	edhash_return_value_t light_out;
	edhash_return_value_t full_out;
	memcpy(&seed, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);
	memcpy(&hash, "~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~~~", 32);

	// Set the difficulty
	edhash_h256_set(&difficulty, 0, 197);
	edhash_h256_set(&difficulty, 1, 90);
	for (int i = 2; i < 32; i++)
		edhash_h256_set(&difficulty, i, 255);
	...
	{
		node node;
		edhash_calculate_dag_item(&node, 0, light);
		const std::string
	...
	}
	{
		for (int i = 0; i < full_size / sizeof(node); ++i) {
			for (uint32_t j = 0; j < 32; ++j) {
				node expected_node;
				edhash_calculate_dag_item(&expected_node, j, light);
				const std::string
	...
	}
	{
		...
		full_out = edhash_full_compute(full, hash, nonce);
		BOOST_REQUIRE(full_out.success);
		light_out = edhash_light_compute_internal(light, full_size, hash, nonce);
		...
		edhash_h256_t check_hash;
		edhash_quick_hash(&check_hash, &hash, nonce, &full_out.mix_hash);
	...
	{
		full_out = edhash_full_compute(full, hash, 5);
		BOOST_REQUIRE(full_out.success);
		std::string
				light_result_string = blockhashToHexString(&light_out.result),
				full_result_string = blockhashToHexString(&full_out.result);
		...
		light_out = edhash_light_compute_internal(light, full_size, hash, 5);
		BOOST_REQUIRE(light_out.success);
		light_result_string = blockhashToHexString(&light_out.result);
		BOOST_REQUIRE_MESSAGE(light_result_string == full_result_string,
		...
		BOOST_REQUIRE_MESSAGE(full_mix_hash_string == light_mix_hash_string,
				"\nlight mix hash: " << light_mix_hash_string.c_str() << "\n"
						<< "full mix hash: " << full_mix_hash_string.c_str() << "\n");
		BOOST_REQUIRE_MESSAGE(edhash_check_difficulty(&full_out.result, &difficulty),
				"edhash_check_difficulty failed"
		);
		BOOST_REQUIRE_MESSAGE(edhash_quick_check_difficulty(&hash, 5U, &full_out.mix_hash, &difficulty),
				"edhash_quick_check_difficulty failed"
		);
	}
	edhash_light_delete(light);
	edhash_full_delete(full);
...
BOOST_AUTO_TEST_CASE(edhash_full_new_when_dag_exists_with_wrong_size) {
	uint64_t full_size;
	uint64_t cache_size;
	edhash_h256_t seed;
	edhash_h256_t hash;
...
BOOST_AUTO_TEST_CASE(test_block22_verification) {
	...
	edhash_light_t light = edhash_light_new(22);
	edhash_h256_t seedhash = stringToBlockhash("372eca2454ead349c3df0ab5d00b0b706b23e49d469387db91811cee0358fc6d");
	BOOST_ASSERT(light);
	edhash_return_value_t ret = edhash_light_compute(
		light,
		seedhash,
		0x495732e0ed7a801cU
	);
	BOOST_REQUIRE_EQUAL(blockhashToHexString(&ret.result), "00000b184f1fdd88bfd94c86c39e65db0c36144d5e43f745f722196e730cb614");
	edhash_h256_t difficulty = edhash_h256_static_init(0x2, 0x5, 0x40);
	BOOST_REQUIRE(edhash_check_difficulty(&ret.result, &difficulty));
	edhash_light_delete(light);
}

BOOST_AUTO_TEST_CASE(test_block30001_verification) {
	// from POC-9 testnet, epoch 1
	edhash_light_t light = edhash_light_new(30001);
	edhash_h256_t seedhash = stringToBlockhash("7e44356ee3441623bc72a683fd3708fdf75e971bbe294f33e539eedad4b92b34");
	BOOST_ASSERT(light);
	edhash_return_value_t ret = edhash_light_compute(
		light,
		seedhash,
	...
	edhash_h256_t difficulty = edhash_h256_static_init(0x17, 0x62, 0xff);
	BOOST_REQUIRE(edhash_check_difficulty(&ret.result, &difficulty));
	edhash_light_delete(light);
}
...
BOOST_AUTO_TEST_CASE(test_block60000_verification) {
	...
	edhash_light_t light = edhash_light_new(60000);
	edhash_h256_t seedhash = stringToBlockhash("5fc898f16035bf5ac9c6d9077ae1e3d5fc1ecc3c9fd5bee8bb00e810fdacbaa0");
	BOOST_ASSERT(light);
	edhash_return_value_t ret = edhash_light_compute(
		light,
		seedhash,
	...
	edhash_h256_t difficulty = edhash_h256_static_init(0x25, 0xa6, 0x1e);
	BOOST_REQUIRE(edhash_check_difficulty(&ret.result, &difficulty));
	edhash_light_delete(light);
}

// Test of Full DAG creation with the minimal edhash.h API.
...
BOOST_AUTO_TEST_CASE(full_dag_test) {
	edhash_light_t light = edhash_light_new(55);
	BOOST_ASSERT(light);
	edhash_full_t full = edhash_full_new(light, progress_cb);
	BOOST_ASSERT(full);
	edhash_light_delete(light);
	edhash_full_delete(full);
}
