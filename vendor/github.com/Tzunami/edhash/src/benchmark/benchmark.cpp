/*
  This file is part of cpp.earthdollar.

  cpp.earthdollar is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  cpp.earthdollar is distributed in the hope that it will be useful,
...
#include <libedhash/util.h>
#ifdef OPENCL
#include <libedhash-cl/edhash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(edhash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		edhash_return_value hash;
		edhash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("edhash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		edhash_return_value hash;
		edhash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : edhash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			edhash_return_value hash;
			edhash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			edhash_return_value hash;
			#ifdef FULL
				edhash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				edhash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = EDHASH_ACCESSES * EDHASH_MIX_BYTES;
