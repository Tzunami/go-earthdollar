...
/*
  ...
  2. https://github.com/Tzunami/cpp.earthdollar/blob/develop/libedhash-cl/edhash_cl_miner.cpp
...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       ged gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        ged --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run ged gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run ged gpuinfo", err)
		}
		for _, d := range ds {
	...
	c.edhash = pow
	dagSize := uint64(C.edhash_get_datasize(C.uint64_t(blockNum)))
	c.dagSize = dagSize

	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: ged gpuinfo")
		} else {
...
func initCLDevice(deviceId int, device *cl.Device, c *OpenCLMiner) error {
	...
	kvs["DAG_SIZE"] = strconv.FormatUint(c.dagSize/edhashMixBytesLen, 10)
	kvs["ACCESSES"] = strconv.FormatUint(edhashAccesses, 10)
	...
	*/
	...
	searchKernelName = "edhash_search"
	hashKernelName = "edhash_hash"
	...
	dagPtr := unsafe.Pointer(c.edhash.Full.current.ptr.data)
...
func (c *OpenCLMiner) Search(block pow.Block, stop <-chan struct{}, index int) (uint64, []byte) {
	c.mu.Lock()
	newDagSize := uint64(C.edhash_get_datasize(C.uint64_t(block.NumberU64())))
...
func (c *OpenCLMiner) Verify(block pow.Block) bool {
	return c.edhash.Light.Verify(block)
}
