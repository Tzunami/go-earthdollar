...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-earthdollar/build/test/createRandomStateTest\" earthdollar/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pyearthdollar
	OUTPUT_PYTHON="$(docker run --rm earthdollar/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm earthdollar/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm earthdollar/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
